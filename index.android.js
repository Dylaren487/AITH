...
export default class game extends Component {
    ...
    constructor() {
        ...
        AsyncStorage.getItem('highScore').then((value) => { this.setState( {highScore: JSON.parse(value)} )})
    }
        ...
        return (
            <View style={containerStyle}>
                    ...
                    <View style={{flex:1}}>
                        <View style={rowStyle}>
                            <View style={columnStyle}>
                                <TouchableHighlight  underlayColor = {'#cccccc'} style={blockStyle} onPress={() => this.handleTouch(0)}>
                                    <Text style={blockTextStyle}>{this.state.holes[0] ? '🐱' : ''}</Text>
                                </TouchableHighlight>
                                <TouchableHighlight  underlayColor = {'#cccccc'} style={blockStyle} onPress={() => this.handleTouch(4)}>
                                    <Text style={blockTextStyle}>{this.state.holes[4] ? '🐱' : ''}</Text>
                                </TouchableHighlight>
                                <TouchableHighlight  underlayColor = {'#cccccc'} style={blockStyle} onPress={() => this.handleTouch(7)}>
                                    <Text style={blockTextStyle}>{this.state.holes[7] ? '🐱' : ''}</Text>
                                </TouchableHighlight>
                            </View>
                            <View style={columnStyle}>
                                <TouchableHighlight  underlayColor = {'#cccccc'} style={blockStyle} onPress={() => this.handleTouch(2)}>
                                    <Text style={blockTextStyle}>{this.state.holes[2] ? '🐱' : ''}</Text>
                                </TouchableHighlight>
                                <TouchableHighlight  underlayColor = {'#cccccc'} style={blockStyle} onPress={() => this.handleTouch(5)}>
                                    <Text style={blockTextStyle}>{this.state.holes[5] ? '🐱' : ''}</Text>
                                </TouchableHighlight>
                                <TouchableHighlight  underlayColor = {'#cccccc'} style={blockStyle} onPress={() => this.handleTouch(8)}>
                                    <Text style={blockTextStyle}>{this.state.holes[8] ? '🐱' : ''}</Text>
                                </TouchableHighlight>
                            </View>
                            <View style={columnStyle}>
                                <TouchableHighlight  underlayColor = {'#cccccc'} style={blockStyle} onPress={() => this.handleTouch(3)}>
                                    <Text style={blockTextStyle}>{this.state.holes[3] ? '🐱' : ''}</Text>
                                </TouchableHighlight>
                                <TouchableHighlight  underlayColor = {'#cccccc'} style={blockStyle} onPress={() => this.handleTouch(6)}>
                                    <Text style={blockTextStyle}>{this.state.holes[6] ? '🐱' : ''}</Text>
                                </TouchableHighlight>
                                <TouchableHighlight  underlayColor = {'#cccccc'} style={blockStyle} onPress={() => this.handleTouch(9)}>
                                    <Text style={blockTextStyle}>{this.state.holes[9] ? '🐱' : ''}</Text>
                                </TouchableHighlight>
                            </View>
                        </View>
                    </View>
                ...
                <View style={footerStyle}>
                    <TouchableOpacity onPress={this.startGame.bind(this)} disabled={this.state.playing} style={[boxStyle,{flex:1}]}>
